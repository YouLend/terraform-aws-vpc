locals {
  ...
  nat_gateway_details = {
    for idx, subnet_id in var.public_subnets : subnet_id => {
      subnet_id     = subnet_id
      allocation_id = local.nat_gateway_ips[idx % length(local.nat_gateway_ips)]
...
resource "aws_route_table" "private" {
  for_each = var.create_vpc && local.max_subnet_length > 0 ? { for subnet_id in var.public_subnets : subnet_id => subnet_id } : {}
  vpc_id = local.vpc_id

  tags = merge(
    {
      "Name" = var.single_nat_gateway ? format("%s-%s", var.name, var.private_subnet_suffix) : format("%s-%s-%s", var.name, var.private_subnet_suffix, each.key)
    },
    var.tags,
...
resource "aws_route_table_association" "private" {
  for_each = var.create_vpc && length(var.private_subnets) > 0 ? { for subnet_id in var.private_subnets : subnet_id => subnet_id } : {}

  subnet_id = each.key
  route_table_id = var.single_nat_gateway ? (element(values(aws_route_table.private), 0).id) : aws_route_table.private[each.key].id
}
